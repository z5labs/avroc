...
What is not yet there, so that it is not mistaken for a gap: a file an earlier run produced that this one did not is not pruned (#119). The generators here still emit chunks internally through the `Generator` service's stream type, reassembled in `internal/plugin`; #121–#123 replace that with a plain write and #124 deletes the service.
