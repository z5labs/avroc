...
- **W3C Trace Context**, *Recommendation* — the normative definition of the
  `traceparent` and `tracestate` **header** value formats, which are the values
  [Trace context](#trace-context) carries and the ones a plugin parses. It
  defines HTTP headers and says nothing about environment variables, so it is
  cited for the format and never for the carrier.
  <https://www.w3.org/TR/trace-context/>
- **OpenTelemetry**, *Environment Variable Specification* — cited for what it
  covers, which is how an **SDK** is configured: endpoint, protocol, headers,
  sampler, resource. Those variables reach a plugin by ordinary inheritance and
  need nothing from this contract. Context *propagation* is not among the things
  it specifies, which is why the carrier below is a convention and not a
  standard.
...
> Reproducible Builds governs timestamps, W3C Trace Context governs the format
> of one pair of values and the OpenTelemetry specification governs an SDK's own
> configuration, and the IR spec governs the bytes. Neither telemetry source
> governs the environment variables [Trace context](#trace-context) names —
> nothing does, and that subsection says so rather than borrowing authority it
> has not got. Where this document appears to contradict
> [`ir/SPEC.md`](../ir/SPEC.md) about the descriptor, that document wins and
> this one has a bug.
...
avroc passes its own environment through to a generator unchanged, except in two
places: as [Determinism](#determinism) requires of `SOURCE_DATE_EPOCH`, and as
[Trace context](#trace-context) requires of `TRACEPARENT` and `TRACESTATE`.
Those are the whole of the list. Every other variable a plugin observes is one
avroc itself was started with, reaching the plugin by ordinary process
inheritance and touched by nothing here.
...
That rule is what decides the shape of the exception below rather than being
bent by it. Trace context configures the *process*, not the output: a plugin
that ignores it entirely is fully conformant, and a plugin that let it change a
generated byte would be breaking [Determinism](#determinism). So what follows is
a second exception to passing the environment through unchanged, and not a
second way to configure a plugin.

#### Trace context

When avroc is tracing, it sets `TRACEPARENT`, and `TRACESTATE` where there is
one, on every process it runs — a generation invocation and a `--plugin-info`
handshake alike, the handshake included because it is a whole process and the
first one in a run that can be slow. Their values are the [W3C Trace Context]
header values of the span avroc opened for *that* invocation, so a plugin that
starts its own spans with them as the parent produces a trace nested under the
run that asked for the work.

The value is **per-invocation**, which is why it is set rather than merely
inherited: it is neither the value avroc was started with nor one value shared
...
- A plugin **MUST NOT** require either variable to be set in order to do its
  normal work. This is the rule above applied rather than a new one, and it
  holds however avroc was configured, because a plugin cannot tell whether the
  variables are absent by choice or by circumstance.
- A plugin **MUST NOT** let either variable affect any byte it writes beneath
  `--out`. [Determinism](#determinism) already forbids it — a trace id is named
  there among the values that may not be embedded — and it is repeated here
  because this is where the temptation arrives: a plugin holding a trace id is
  holding a unique-looking string, and two invocations of one run carry
  different ones.

Their **absence is ordinary, not exceptional**, and a plugin has to handle it as
the common case:

- avroc is frequently not tracing at all, and then it sets neither variable —
  **including when its own environment carried one**. Forwarding an inherited
  value would parent a plugin's spans to a trace avroc is not part of, so both
  are removed from the child's environment rather than passed through. A plugin
  therefore never sees trace context that does not belong to the invocation it
  is running.
- A plugin that finds neither variable **MUST NOT** treat that as an error, and
  **MUST NOT** decline to generate. It starts an unparented trace of its own or
  starts no trace at all; both are conformant, and which one is right is a
  property of the plugin and not of this contract.
- A plugin run by something that is not avroc — a shell, a pipeline, a test
  harness — is in exactly that position, and so is one whose author never
  thought about tracing. Following a convention the ecosystem already has is
  what makes those the same case rather than three.

**The carrier is a convention, and this document cites it as one.**
`TRACEPARENT` and `TRACESTATE` are not defined by any specification. They derive
from two that are real and neither of which says this: [W3C Trace Context]
defines the `traceparent` and `tracestate` **HTTP headers**, whose value formats
are what avroc writes and what a plugin parses, and says nothing about
environment variables; the [OpenTelemetry environment variable
specification][otel-env] defines how an **SDK is configured** from the
environment and does not cover context propagation. The convention is those
header names upper-cased and carried across a process boundary instead of a
network one, and its weight is entirely its adopters — [`otel-cli`][otel-cli],
[Dagger][dagger], [Buildkite][buildkite] and the [Jenkins OpenTelemetry
plugin][jenkins] all read or write this pair. avroc follows them instead of
naming a variable of its own, which is why avroc running under `dagger call`
already has a parent, and why a plugin that already speaks the convention needs
...
plugin by **ordinary inheritance**: it is avroc's own environment, passed
through, with avroc configuring none of it for the child. There is no mechanism
here to go looking for and nothing to opt into. A plugin that exports spans
configures its SDK from the environment exactly as it would had a shell run it,
and the operator who pointed avroc at a collector has pointed the plugin at the
same one. `OTEL_SERVICE_NAME` is included in that: avroc neither sets nor
overrides it, so a user who sets it gets the whole build under one service name
and a user who does not gets each executable's own default.
...
[dagger]: https://docs.dagger.io/features/traces
[buildkite]: https://buildkite.com/docs/pipelines/integrations/observability/opentelemetry
...
absolute path, a trace id — including one taken from the
[trace context](#trace-context) avroc handed it — or a random value in its
output, and **MUST** emit anything
...
| [The environment](#the-environment), [Trace context](#trace-context) | [#193](https://github.com/z5labs/avroc/issues/193), [#194](https://github.com/z5labs/avroc/issues/194) |
