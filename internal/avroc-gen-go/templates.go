...
const filePreamble = `// Code generated by avroc-gen-go. DO NOT EDIT.

package %s
`

// avrolibImport is the one non-standard import this generator ever writes, and
// it is aliased because the package's own name is avro while its path ends in
// avro-go.
const avrolibImport = `avro "github.com/z5labs/avro-go"`

// fileImports is the set of imports one generated file needs.
//
// It is a set of booleans rather than a list because the emission order is not
// the discovery order: the standard library group is written in the order below,
// which is already alphabetical, so the block gofmt would produce is the block
// that is written and go/format never has to move a line. A list appended to as
// each need is found would order the imports by which feature happened to be
// generated first, which is a way for two runs over the same descriptor to
// disagree.
...
// writePreamble writes the generated-code banner, the package clause and the
// import block, in that order.
func (f fileImports) writePreamble(cb *codeBuilder, packageName string) {
	cb.writef(filePreamble, packageName)
...
func generateFileCode(packageName string, schema *avrocpb.Schema, singleObject bool, fp [8]byte) string {
	...
	imports := fileImports{
		...
		io:      stream != nil,
		iter:    stream != nil,
		avrolib: needsAvrolibImport(types) || stream != nil,
	}
	imports.writePreamble(cb, packageName)
