...
// outlived them went with #123 — Generate writes whole files through
// plugin.FileWriter, and nothing here names the Generator service any more.
//
// Whole files rather than chunks matter more for this generator than for the
// others. Its output is a Parsing Canonical Form, which is the input to a
// schema fingerprint: the file is a value read byte for byte and not source a
// tool will reformat, so the write must add nothing — no trailing newline, no
// re-indentation. plugin.FileWriter writes the content it is handed and nothing
// else, and TestGeneratedFileIsExactlyTheCanonicalForm holds that.
//
...
func Main(ctx context.Context, c cli.Context) int {
	return plugin.Main(ctx, c, plugin.NewInfo("pcf", declaredOptions()...), Generate)
}
